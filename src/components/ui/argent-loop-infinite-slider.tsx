...
export function Component() {
  ...
  React.useEffect(() => {
    ...
    // Passive false because we preventDefault
  ...
  return (
    <div className="parallax-container overflow-hidden w-full relative">
      <ul className="project-list absolute inset-0 w-full h-full m-0 p-0 list-none">
        {indices.map((i) => {
          const data = getProjectData(i);
          return (
            <div
              key={i}
              className="project absolute top-0 left-0 w-full h-full flex items-center justify-center overflow-hidden"
              ref={(el) => {
                if (el) projectsRef.current.set(i, el);
                else projectsRef.current.delete(i);
              }}
            >
              <img className="object-cover w-full h-full" src={data.image} alt={data.title} />
            </div>
          );
        })}
      </ul>

      <div className="minimap absolute bottom-12 right-12 flex gap-8 z-10 pointer-events-none">
        <div className="minimap-wrapper flex gap-4 h-[250px] overflow-hidden relative">
          <div className="minimap-img-preview w-[160px] relative">
            {indices.map((i) => {
              const data = getProjectData(i);
              return (
                <div
                  key={i}
                  className="minimap-img-item absolute top-0 left-0 w-full h-[250px] overflow-hidden rounded-md"
                  ref={(el) => {
          ...
          <div className="minimap-info-list w-[200px] relative">
            {indices.map((i) => {
              const data = getProjectData(i);
              ...
              return (
                <div
                  key={i}
                  className="minimap-item-info absolute top-0 left-0 w-full h-[250px] flex flex-col justify-center text-white mix-blend-difference"
                  ref={(el) => {
                    if (el) infoRef.current.set(i, el);
                    else infoRef.current.delete(i);
                  }}
                >
                  <div className="minimap-item-info-row flex justify-between uppercase text-xs tracking-widest border-b border-white/20 pb-2 mb-2">
                    <p>{num}</p>
                    <p>{data.title}</p>
                  </div>
                  <div className="minimap-item-info-row flex justify-between text-xs opacity-70 mb-2">
                    <p>{data.category}</p>
                    <p>{data.year}</p>
                  </div>
                  <div className="minimap-item-info-row text-xs italic opacity-50">
